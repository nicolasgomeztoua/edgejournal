...
export default function AIInsightsPage() {
	...
	const { data: trades } = api.trades.getAll.useQuery({ status: "closed", limit: 100 });
	...
	const generateLocalInsight = (query: string): string => {
		...
		if (lowerQuery.includes("breakeven") || lowerQuery.includes("break even")) {
			...
			return `**Breakeven Analysis:**\n\n` +
				`Out of ${stats.totalTrades} trades, ${stats.breakevens} ended at breakeven (${beRate.toFixed(1)}%).\n\n` +
				`**Insight:** ${beRate > 10 
		...
		if (lowerQuery.includes("setup") || lowerQuery.includes("strategy")) {
			...
			const setupSummary = Object.entries(setupStats)
				.map(([setup, data]) => {
					...
					return `- **${setup}:** ${wr}% win rate, ${formatCurrency(data.pnl)} P&L`;
				})
				.join("\n");

			return `**Setup Performance:**\n\n${setupSummary}\n\n` +
		...
		if (lowerQuery.includes("symbol") || lowerQuery.includes("instrument")) {
			...
			const symbolSummary = Object.entries(symbolStats)
				...
				.map(([symbol, data]) => {
					const total = data.wins + data.losses;
					const wr = total > 0 ? (data.wins / total * 100).toFixed(1) : "0";
					return `- **${symbol}:** ${wr}% win rate, ${formatCurrency(data.pnl)} P&L`;
				})
				.join("\n");

			return `**Performance by Symbol:**\n\n${symbolSummary}\n\n` +
				`**Insight:** Focus on your best performing symbols. Consider if you understand certain markets better than others.`;
		}

		// Profit factor analysis
		if (lowerQuery.includes("profit factor") || lowerQuery.includes("r:r") || lowerQuery.includes("risk")) {
			return `**Risk Analysis:**\n\n` +
				...
				`- Avg R:R: ${stats.avgLoss > 0 ? (stats.avgWin / stats.avgLoss).toFixed(2) : "N/A"}\n\n` +
				`**Insight:** ${stats.profitFactor >= 1.5 
